...
def build():
    def exit():
        exit = messagebox.askyesno("Cryptography", "Confirm you want to exit to open the main application?")
        if exit > 0:
            window.destroy()
            return
    window = tk.Tk()
    window.title("Welcome to Cryptoography")
    window.geometry("600x300")
    bg = tk.PhotoImage(file="auth.png")
    canvas1 = tk.Canvas(window, width=400, height=400)
    canvas1.pack(fill="both", expand=True)
    canvas1.create_image(0, 0, image=bg, anchor="nw")
    l1 = tk.Label(canvas1, text="Welcome To Cryptography Project. \nThis project is based on substitution encryption "
                               "technique.", font=("Pristina", 16))
    l1.pack(padx=16, pady=5)
    b2 = tk.Button(canvas1, text="Authenticate", font=("Cascadia Code", 18), bg="green", fg="orange", command=ultimate)
    b2.pack(padx=16, pady=16)
    b3 = tk.Button(canvas1, text="Exit", font=("Cascadia Code", 18), bg="green", fg="yellow", command=exit)
    b3.pack(padx=16, pady=16)
    window.mainloop()
    return text
